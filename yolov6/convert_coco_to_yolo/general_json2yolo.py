...
def convert_coco_json(json_dir='../../coco/annotations/', use_segments=False, cls91to80=False):
    ...
    for json_file in sorted(Path(json_dir).resolve().glob('*.json')):
        ...
        fn = Path(save_dir) / 'labels' / json_file.stem.replace('set_3_', '')  # folder name
        fn.mkdir()
        # print(json_file)
        with open(json_file) as f:
            data = json.load(f)
...
if __name__ == '__main__':
    ...
    if source == 'COCO':
        convert_coco_json('/media/nsl6/hdd1/mredul/object_detection/YOLOv6/convert_coco_to_yolo/object_detection_data/annotations',  # directory with *.json
                          use_segments=False, #True,
                          cls91to80=False)
    # exit(0)
